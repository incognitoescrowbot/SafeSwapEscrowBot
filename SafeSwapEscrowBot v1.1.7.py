...
def check_duplicate_description(user_id, description):
    """
    Check if a user has already used this description for a previous transaction.
    ...
    Returns:
        bool: True if description is a duplicate, False if it's unique
    """
    conn = None
    try:
        conn = sqlite3.connect(DB_PATH, timeout=20.0)
...
async def confirm_transaction(update: Update, context: CallbackContext) -> int:
    ...
    if check_duplicate_description(user.id, description):
        await update.message.reply_text(
            ...
            "You have already used this description for a previous transaction. "
        ...
        if balance_btc >= threshold_99:
            ...
            if is_buyer:
                await update.message.reply_text(
                    ...
                    f"Wallet balance: *{balance_btc:.8f} BTC*\n\n"
